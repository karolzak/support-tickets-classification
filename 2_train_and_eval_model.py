...
if __name__ == '__main__':  
    ...
    dfTickets = pd.read_csv('./datasets/all_tickets.csv', dtype=str) # loading dataset from csv

    #categorical_columns=""  # this is for future work
    ...
    train_data, test_data, train_labels, test_labels = train_test_split(data,labelData, test_size=0.2) # split data to train/test sets with 80:20 ratio
    #train_df, test_df = train_test_split(dfTickets, test_size=0.2)
    ...
    if classifier == "NB":
        ...
        text_clf = Pipeline([('vect', count_vect), ('tfidf', TfidfTransformer()),
    ...
    #plt.savefig(os.path.join('.', 'outputs', 'confusion_matrix.png')) # Save confusion matrix to outputs in Workbench
    plt.show()
    ...
    # Use with Workbench
    if use_grid_search:
        pickle.dump(gs_clf, open(os.path.join('.', 'outputs', dataset_name+".model" ), 'wb') )    
    else :
        pickle.dump(text_clf, open(os.path.join('.', 'outputs', dataset_name+".model" ), 'wb') )    
