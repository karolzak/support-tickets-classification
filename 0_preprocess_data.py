...
def removeNonEnglish(text, englishWords):
    ...
    result=x[maskedArr]
    text=np.array2string(result).replace("\'","").replace("[","").replace("]","").replace("\n","").replace("\r","")
...
if __name__ == '__main__':  
    ...
    # Reorder columns
    columnsOrder= ['title','body','ticket_type','category','sub_category1','sub_category2','business_service','urgency','impact']
    ...
    dfTickets=dfRequests.append(dfIncidents,ignore_index=True) # ignore_index=True to avoid index duplicates
    ...
    #dfTickets['body'] = (dfTickets['title']+ " " + dfTickets['body']).map(str)
    ...
    print("Shape before removing blacklisted words from english ords dataset: "+str(dfWords.shape))
    dfWords = dfWords.merge(dfBlackListWords.drop_duplicates(), on=['Line'], how='left', indicator=True) 
    # Select words
    dfWords=dfWords.loc[dfWords['_merge'] == 'left_only']
    ...
    dfTickets['body']=dfTickets['body'].apply(lambda emailBody: removeNonEnglish(emailBody,dfWords['Line']))
    dfTickets['title']=dfTickets['title'].apply(lambda emailBody: removeNonEnglish(emailBody,dfWords['Line']))
    ...
    columnsToEncrypt = ['category','sub_category1','sub_category2','business_service','urgency','impact','ticket_type']
